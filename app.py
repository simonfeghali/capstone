...
# =============================================================================
# CAPEX TAB — line chart becomes a KPI when a year is chosen
# =============================================================================
with tab_eda:
    ...
    # A placeholder right under the Grade filter for the single KPI (used when a specific year is chosen)
    ...
    # NEW ✅: When a specific year is chosen, show a single KPI (and no left trend)
    if isinstance(sel_year_any, int):
        total_capex_sel = float(capx_eda["capex"].sum()) if not capx_eda.empty else np.nan
        scope_label = (
            sel_country if sel_country != "All"
            else (sel_cont if sel_cont != "All" else "Global")
        )
        below_grade_kpi.markdown(
            f"""
            <div class="kpi-box">
              <div class="kpi-title">{scope_label} — Total CAPEX — {sel_year_any}</div>
              <div class="kpi-number">{'-' if np.isnan(total_capex_sel) else f'{total_capex_sel:,.3f}'}</div>
    ...
    with e1:
        # If a specific year is picked, we skip the left trend (replaced by the KPI above)
        if not isinstance(sel_year_any, int):
            ...
            if trend.empty:
                ...
                title = (f"{sel_country} CAPEX Trend ($B)" if sel_country != "All"
                         else ("{0} CAPEX Trend ($B)".format(sel_cont) if sel_cont != "All"
    ...
    # Grade views
    hide_grade_view = False  # unchanged logic, but keep name for clarity
                    ...
                    if nonzero.shape[0] <= 1:
                        if nonzero.empty:
                            ...
                            _kpi_block(f"CAPEX by Grade — {sel_year_any} — {nonzero['grade'].iloc[0]}",
                                       float(nonzero["capex"].iloc[0]) / 1000.0, "$B")
                    else:
                        fig = px.bar(gb_sorted.assign(capex=lambda d: d["capex"]/1000.0),
                                     x="capex", y="grade", orientation="h",
                                     labels={"capex": "", "grade": ""},
