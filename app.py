...
st.markdown(
    ...
    <style>
      ...
      .kpi-big {
        font-size: 64px; font-weight: 800; line-height: 1.1; text-align:center;
      }
      .kpi-title {
        font-weight: 700; font-size: 18px; margin-bottom: 0.35rem;
      }
    </style>
    """,
...
# ──────────────────────────────────────────────────────────────────────────────
# Load World Bank (Scoring) and CAPEX EDA
# ──────────────────────────────────────────────────────────────────────────────
...
SELECTED_COUNTRIES_10 = [
    "United States", "United Kingdom", "France", "Germany", "Japan",
    "China", "South Korea", "Canada", "Netherlands", "UAE"
]
...
def load_sectors() -> pd.DataFrame:
    ...
    ctry = find_col(df.columns, "country")
    sect = find_col(df.columns, "sector")
    comp = find_col(df.columns, "companies", "company", "num_companies", "companies_count")
    jobs = find_col(df.columns, "jobs created", "jobs_created", "jobs")
    cap  = find_col(df.columns, "capex", "capex ($b)", "capital_expenditure")
    proj = find_col(df.columns, "projects", "num_projects", "project_count")

    for need, col in [("country", ctry), ("sector", sect)]:
        if col is None:
    ...
    df = df.rename(columns={
        ...
        **({cap: "capex"} if cap else {}),
        **({proj: "projects"} if proj else {}),
    })

    # Robust numerics (handles 1,234 and blanks)
    for col in ["companies", "jobs_created", "capex", "projects"]:
...
# Auto-continent suggestion if a country already selected
...
with tab_scoring:
    ...
    # Indicator Weights table — SHOWN ONLY IN SCORING
...
# =============================================================================
# EDA TAB (CAPEX)  (UNCHANGED, including KPI behavior for single year + country)
# =============================================================================
with tab_eda:
    st.caption("Exploratory Data Analysis • (CAPEX)")

    # EDA-only Grade selector (auto when country+specific year)
    ...
    # If year is specific keep that year; if 'All' keep all
    if isinstance(sel_year_any, int):
        capx_eda = capx_eda[capx_eda["year"] == sel_year_any]

    # If country specific & year specific ⇒ KPI instead of trend
    if sel_country != "All" and isinstance(sel_year_any, int):
        # KPI for selected country + year
        this_val = float(capx_eda["capex"].sum()) if not capx_eda.empty else np.nan
        k1, k2 = st.columns([1, 2], gap="large")
        with k1:
            st.markdown(f"<div class='kpi-title'><b>{sel_country} CAPEX — {sel_year_any}</b></div>", unsafe_allow_html=True)
            st.markdown(
                f"<div class='kpi-big'>{'-' if np.isnan(this_val) else f'{this_val:,.1f}'}<span style='font-size:22px'> $B</span></div>",
                unsafe_allow_html=True
            )
        with k2:
            # map next to KPI
        ...
        # TOP ROW: Country/Continent/Global CAPEX trend + map
        ...
        with e1:
            ...
            if trend.empty: 
                ...
                title = (f"{sel_country} CAPEX Trend" if sel_country != "All"
                         else (f"{sel_cont} CAPEX Trend" if sel_cont != "All" else "Global CAPEX Trend"))
...
with tab_sectors:
    ...
    # Controls (sector shows ALL sectors; country limited to notebook’s 10)
    all_sectors = ["All"] + sorted(sectors_df["sector"].dropna().unique().tolist())
    colS, colC = st.columns([1, 2], gap="large")
    with colS:
        sel_sector = st.selectbox("Sector", all_sectors, index=0, key="sector_sel")
    with colC:
        sel_country_sect = st.selectbox("Country (Sectors)", sorted(SELECTED_COUNTRIES_10), index=0, key="sector_country")

    # Metric radio
    metric = st.radio(
        "Metric",
        options=["Companies", "Jobs Created", "Capex", "Projects"],
        horizontal=True,
        index=0,
    )
    ...
    mcol = metric_col_map[metric]

    # Base filter to just the 10 notebook countries
    sf = sectors_df[sectors_df["country"].isin(SELECTED_COUNTRIES_10)].copy()

    # Country filter (always required)
    sf = sf[sf["country"] == sel_country_sect]

    # If Sector == All -> show bar across SELECTED_SECTORS ONLY
    if sel_sector == "All":
        plot_df = (
            sf[sf["sector"].isin(SELECTED_SECTORS)]
            .groupby("sector", as_index=False)[mcol].sum()
            .sort_values(mcol, ascending=False)
        )
        title = f"{metric} by Sector — {sel_country_sect}"
        if plot_df.empty:
            st.info("No data for this selection.")
        else:
            fig = px.bar(
                plot_df, x=mcol, y="sector",
                orientation="h",
        ...
        # Specific sector + country -> KPI
        this_val = float(sf.loc[sf["sector"] == sel_sector, mcol].sum()) if not sf.empty else np.nan
        st.markdown(f"<div class='kpi-title'><b>{sel_country_sect} — {sel_sector} • {metric}</b></div>", unsafe_allow_html=True)
        unit = "" if metric in ("Companies", "Projects") else (" $B" if metric == "Capex" else "")
        st.markdown(
            f"<div class='kpi-big'>{'-' if np.isnan(this_val) else f'{this_val:,.0f}' if unit=='' else f'{this_val:,.0f}'}"
            f"<span style='font-size:22px'>{unit}</span></div>",
            unsafe_allow_html=True
        )
# app.py
import numpy as np
