...
with tab_eda:
    ...
    # KPI: Top Country by CAPEX (replaces "Top 10 Countries by CAPEX" chart)
    with b1:
        if isinstance(sel_year_any, int):
            level_df = capx_eda.copy()
            period_label = f"{sel_year_any}"
        else:
            level_df = capx_eda.groupby("country", as_index=False)["capex"].sum()
            period_label = "All Years"
        if level_df.empty or level_df["capex"].fillna(0).sum() == 0:
            st.info("No CAPEX data for this selection.")
        else:
            # pick top country
            top_row = level_df.dropna(subset=["capex"]).sort_values("capex", ascending=False).head(1)
            top_country = str(top_row["country"].iloc[0]) if "country" in top_row.columns else (sel_country if sel_country != "All" else "N/A")
            top_value = float(top_row["capex"].iloc[0]) if not top_row.empty else 0.0
            st.markdown(
                f"""
                <div class="kpi-box">
                  <div class="kpi-title">Top CAPEX Country — {period_label}</div>
                  <div class="kpi-number">{top_value:,.1f}</div>
                  <div class="kpi-sub">$B • {top_country}</div>
                </div>
                """,
                unsafe_allow_html=True,
            )
    ...
    if show_grade_trend:
        with b2:
            if "grade" in capx_enriched.columns:
                if isinstance(sel_year_any, int):
                    # Bar chart, highest at the top
                    gb = (capx_enriched
                          .loc[
                              (capx_enriched["year"] == sel_year_any) &
                    ...
                    if gb.empty:
                        st.info("No CAPEX data by grade.")
                    else:
                        fig = px.bar(
                            gb, x="capex", y="grade", orientation="h",
                            labels={"capex": "", "grade": ""},
                        ...
                        fig.update_yaxes(categoryorder="array", categoryarray=gb["grade"].tolist())
                        fig.update_layout(margin=dict(l=10, r=10, t=60, b=10), height=420)
                        st.plotly_chart(fig, use_container_width=True)
                else:
    ...
    # KPI: Top CAPEX Growth (replaces growth bar chart)
    with (b3 if show_grade_trend else b3):
        ...
        if growth_base.empty:
            ...
            if first_year is None or last_year is None or first_year == last_year:
                ...
                if joined.empty:
                    st.info("No growth data for this selection.")
                else:
                    top_g = joined.sort_values("growth_abs", ascending=False).head(1)
                    top_g_country = str(top_g["country"].iloc[0])
                    top_g_value = float(top_g["growth_abs"].iloc[0])
                    st.markdown(
                        f"""
                        <div class="kpi-box">
                          <div class="kpi-title">Top CAPEX Growth [{first_year} → {last_year}]</div>
                          <div class="kpi-number">{top_g_value:,.1f}</div>
                          <div class="kpi-sub">$B • {top_g_country}</div>
                        </div>
                        """,
                        unsafe_allow_html=True,
                    )
...
with tab_sectors:
    ...
    if sel_sector == "All":
        ...
        bars = bars.sort_values(value_col, ascending=False)  # descending
